...
3. Start oravo by running `podman-compose up -d`.

If you need to stop oravo, you can do so by running `podman-compose down`.


### Install systemd service (optional)

If you want to install a systemd service to run oravo, you can use the provided
systemd service.

Edit `oravo.service` and change these two variables:


	WorkingDirectory=/opt/apps/oravo
	EnvironmentFile=/opt/apps/oravo/.env
...
that oravo is not currently running.  If you started it previously, bring it
down using:
